...
def main():
    ...
    all_fwd = args.forward_fp.split(',')
    all_rev = args.reverse_fp.split(',')

    for (fwd_fp, rev_fp) in zip(all_fwd,all_rev):
        with open(fwd_fp.strip()) as fwd, open(rev_fp.strip()) as rev:
            while True:
                try:
                    # iterate total reads
